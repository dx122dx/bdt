...
A general-purpose **Minecraft 1.20.1 Fabric client-side** infrastructure mod (mod id: `infrastructure`).
...
> Design philosophy: keep infrastructure decoupled from business logic. Debug tooling for a specific mod is injected by **that mod** (or a companion mod) through the `infrastructure:debugger` extension point, and is silently skipped when the target mod is absent — never slowing down or blocking game startup.
...
- **Dual-track config**:
  - Fixed options (verbose logging, failure stack traces) go through AutoConfig → `config/infrastructure.json`.
  - Dynamic feature toggles go through a separate Gson store → `config/infrastructure-features.json` (kept apart from AutoConfig so dynamic fields aren't dropped).
...
1. Download `infrastructure-<version>.jar` from the releases page.
...
Root command is `/inf` (also `/infrastructure`).
...
The core of `infrastructure` is a lightweight module framework in the `core` package:
...
- Enabled state is persisted by `FeatureStateStore` to `config/infrastructure-features.json` and survives restarts.
...
- An external mod implements `DebuggerBuiltinProvider` and registers a custom entrypoint `"infrastructure:debugger"` in its `fabric.mod.json`;
- In `contribute(contributor)`, it calls `contributor.add(requiredModId, displayName, XxxBuiltins::register)`;
- `infrastructure` only runs `register` when `requiredModId` is loaded, otherwise the whole pack is skipped.
...
| --- | --- | --- |
| General | Verbose logging, show stack trace on failure | `config/infrastructure.json` (AutoConfig) |
| Debug Features | Per-feature toggles (decided at runtime) | `config/infrastructure-features.json` (Gson) |
...
Your mod only needs `modImplementation infrastructure` (required at runtime) and implements the extension point:
...
- Use your own namespace for action/feature ids (e.g. `your-mod-id:foo`); don't occupy `infrastructure`.
...
Artifacts land in `build/libs/infrastructure-<version>.jar`.
...
Released under **AGPL-3.0-only**. Source code: <https://github.com/dx122dx/mod-infrastructure>.
