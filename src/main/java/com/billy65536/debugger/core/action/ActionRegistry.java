package com.billy65536.debugger.core.action;
...
public final class ActionRegistry {
    ...
    public static void register(IDebugAction action) {
        if (action == null || action.getId() == null) {
            BdtMod.LOGGER.warn("Attempted to register null action or action with null ID, ignored");
            return;
        }
        Identifier id = action.getId();
        if (actions.containsKey(id)) {
            BdtMod.LOGGER.warn("Debug action {} is already registered, overwriting", id);
        }
        actions.put(id, action);
        BdtMod.LOGGER.info("Registered debug action: {}", id);
    }
