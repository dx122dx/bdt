package com.billy65536.debugger.mixin;
 ...
 * 不可引用本模组自身的 {@code BdtMod} 等尚未初始化的类。</p>
 */
public class BdtMixinPlugin implements IMixinConfigPlugin {

    private static final Logger LOGGER = LoggerFactory.getLogger("bdt-mixin");

    /** 内置包 Mixin 的包前缀 → 所需目标模组 id。 */
    private static final String CHUNKSCANNER_MIXIN_PKG = "com.billy65536.debugger.builtin.chunkscanner.mixin";

    private static final String CHUNKSCANNER_MOD_ID = "chunkscanner";
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        if (mixinClassName != null && mixinClassName.startsWith(CHUNKSCANNER_MIXIN_PKG)) {
    ...
    private boolean isChunkScannerLoaded() {
        ...
        if (cached == null) {
            cached = FabricLoader.getInstance().isModLoaded(CHUNKSCANNER_MOD_ID);
            chunkscannerLoaded = cached;
            if (!cached) {
                // 初始化期各条件只判定一次，无日志 spam 风险
