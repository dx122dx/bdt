...
import com.billy65536.debugger.config.DebugToolsConfigScreen;
import com.billy65536.debugger.config.DebuggerConfigLoader;
import com.billy65536.debugger.core.action.ActionRegistry;
import com.billy65536.debugger.core.action.ArgTokenizer;
import com.billy65536.debugger.core.action.IDebugAction;
...
/**
 * {@code /bdt} 命令树的构建与执行逻辑。
 *
 * <p>本模组独立注册自己的根命令，不挂靠任何其他模组的命令树。</p>
 *
 * <p>提供的子命令：</p>
 * <ul>
 *   <li>{@code /bdt action run <id> [args...]} —— 执行调试动作</li>
 *   <li>{@code /bdt action info <id>} —— 查询调试动作的元信息</li>
 *   <li>{@code /bdt feat about <id>} —— 查询调试特性的启用状态</li>
 *   <li>{@code /bdt feat enable|disable <id>} —— 启用/禁用调试特性</li>
 *   <li>{@code /bdt list} —— 列出全部已注册项</li>
 *   <li>{@code /bdt gui} —— 打开配置界面</li>
 * </ul>
 */
public final class BdtCommands {
    ...
    /**
     * 构建 {@code <rootName> ...} 命令树。
     *
     * <p>Brigadier 不支持真正的别名，每个根名都需独立构建一次命令树。</p>
     *
     * @param rootName 根命令名（"bdt" 或 "billysdebugtools"）
     */
    public static LiteralArgumentBuilder<FabricClientCommandSource> buildCommands(String rootName) {
        var root = ClientCommandManager.literal(rootName);

        // ===== /bdt action run <id> [args...] + action info <id> =====
        ...
        // ===== /bdt feat about|enable|disable <id> =====
    ...
    private static int runAction(MinecraftClient client, Identifier rawId, String rawArgs) {
        ...
        if (action == null) {
            sendMsg(client, Text.translatable("bdt.msg.action_not_found", id.toString())
        ...
        if (DebuggerConfigLoader.get().verboseLogging) {
            BdtMod.LOGGER.info("Executing debug action {} with {} arg(s)", id, args.length);
        }

        // 调试代码稳定性天然偏低，异常必须捕获，绝不允许逸出到 Brigadier
        try {
            action.execute(client, args);
            sendMsg(client, Text.translatable("bdt.msg.action_success", id.toString())
                    .formatted(Formatting.GREEN));
            return 1;
        } catch (Exception e) {
            BdtMod.LOGGER.error("Debug action {} failed", id, e);
            String reason = (e.getMessage() != null) ? e.getMessage() : e.getClass().getSimpleName();
            sendMsg(client, Text.translatable("bdt.msg.action_failed", id.toString(), reason)
                    .formatted(Formatting.RED));
    ...
    private static int showAction(MinecraftClient client, Identifier rawId) {
        ...
        sendMsg(client, Text.translatable("bdt.msg.action_info_id",
                        Text.literal(id.toString()).formatted(Formatting.GOLD))
                .formatted(Formatting.GRAY));
        sendMsg(client, Text.literal("  ")
                .append(Text.translatable("bdt.msg.action_info_name")
                        .formatted(Formatting.DARK_GRAY))
                .append(action.getName().copy().formatted(Formatting.AQUA)));
        sendMsg(client, Text.literal("  ")
                .append(Text.translatable("bdt.msg.action_info_desc")
                        .formatted(Formatting.DARK_GRAY))
    ...
    private static int showFeature(MinecraftClient client, Identifier rawId) {
        ...
        if (feature == null) {
            sendMsg(client, Text.translatable("bdt.msg.feature_not_found", id.toString())
                    .formatted(Formatting.RED));
            return 0;
        }
        boolean active = FeatureRegistry.isEnabled(id);
        sendMsg(client, Text.translatable("bdt.msg.feature_status",
                        Text.literal(id.toString()).formatted(Formatting.GOLD),
                        statusText(active))
                .formatted(Formatting.GRAY));
        sendMsg(client, Text.literal("  ")
    ...
    private static int setFeature(MinecraftClient client, Identifier rawId, boolean value) {
        ...
        if (!changed) {
            sendMsg(client, Text.translatable("bdt.msg.feature_unchanged",
                            Text.literal(id.toString()).formatted(Formatting.GOLD),
                            statusText(value))
        ...
        String key = value
                ? "bdt.msg.feature_enabled"
                : "bdt.msg.feature_disabled";
    ...
    private static int listAll(MinecraftClient client) {
        // Action 分节
        sendMsg(client, Text.translatable("bdt.msg.list_actions_title",
                ActionRegistry.size()).formatted(Formatting.GOLD, Formatting.BOLD));
        if (ActionRegistry.size() == 0) {
            sendMsg(client, Text.literal("  ")
                    .append(Text.translatable("bdt.msg.list_empty")
                            .formatted(Formatting.DARK_GRAY)));
        } else {
        ...
        // Feature 分节
        sendMsg(client, Text.translatable("bdt.msg.list_features_title",
    ...
    private static Text statusText(boolean active) {
        return active
                ? Text.translatable("bdt.msg.state_enabled").formatted(Formatting.GREEN)
                : Text.translatable("bdt.msg.state_disabled").formatted(Formatting.GRAY);
    }
     ...
     * {@code minecraft} 或缺失的裸名归一到 {@code bdt}，
     * 已显式带 {@code bdt:} 的则原样保留。</p>
     */
    private static Identifier normalizeIdentifier(Identifier id) {
        if (id == null) return BdtMod.id("unknown");
        if (BdtMod.MOD_ID.equals(id.getNamespace())) return id;
        // 裸名（minecraft 默认命名空间）或未匹配其它命名空间时，归一到本模组命名空间
        return new Identifier(BdtMod.MOD_ID, id.getPath());
    }
}
