package com.billy65536.debugger.builtin;

import java.util.List;

import com.billy65536.debugger.BdtMod;
import com.billy65536.debugger.builtin.chunkscanner.ChunkScannerBuiltins;
    ...
    public static void registerAll() {
        ...
        for (BuiltinPack pack : PACKS) {
            if (!loader.isModLoaded(pack.requiredModId())) {
                BdtMod.LOGGER.info("Builtin pack '{}': skipped (mod '{}' not loaded)",
                        pack.displayName(), pack.requiredModId());
                continue;
            }
            try {
                pack.entry().run();
                BdtMod.LOGGER.info("Builtin pack '{}': registered", pack.displayName());
            } catch (Throwable t) {
                BdtMod.LOGGER.error("Builtin pack '{}': registration failed, skipping",
                        pack.displayName(), t);
            }
        }
    }
