...
/**
 * 模组主配置界面：ModMenu 与 {@code /bdt gui} 的统一入口。
 *
 * <p>包含两个分类：</p>
 * <ul>
 ...
 * {@code config/bdt.json} 与 {@code config/bdt-features.json}。
...
public final class DebugToolsConfigScreen {
    ...
    public static Screen create(Screen parent) {
        ConfigBuilder builder = ConfigBuilder.create()
                ...
                .setTitle(Text.translatable("bdt.gui.title"));
    ...
    private static void buildGeneralCategory(ConfigBuilder builder, ConfigEntryBuilder entryBuilder) {
        ConfigCategory category = builder.getOrCreateCategory(
                Text.translatable("bdt.gui.category.general"));

        category.addEntry(entryBuilder
                .startBooleanToggle(Text.translatable("bdt.gui.option.verbose_logging"),
                        DebuggerConfigLoader.get().verboseLogging)
                .setDefaultValue(false)
                .setTooltip(Text.translatable("bdt.gui.option.verbose_logging.desc"))
                .setSaveConsumer(value -> DebuggerConfigLoader.get().verboseLogging = value)
                .build());

        category.addEntry(entryBuilder
                .startBooleanToggle(Text.translatable("bdt.gui.option.show_action_stack_trace"),
                        DebuggerConfigLoader.get().showActionStackTrace)
                .setDefaultValue(false)
                .setTooltip(Text.translatable("bdt.gui.option.show_action_stack_trace.desc"))
    ...
    private static void buildFeaturesCategory(ConfigBuilder builder, ConfigEntryBuilder entryBuilder) {
        ConfigCategory category = builder.getOrCreateCategory(
                Text.translatable("bdt.gui.category.features"));

        if (FeatureRegistry.size() == 0) {
            category.addEntry(entryBuilder
                    .startTextDescription(Text.translatable("bdt.gui.no_features"))
                    .build());
            return;
        }
