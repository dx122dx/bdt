...
public final class ActionRegistry {
    ...
    public static void register(IDebugAction action) {
        if (action == null || action.getId() == null) {
            CsDebuggerMod.LOGGER.warn("Attempted to register null action or action with null ID, ignored");
            return;
        }
        Identifier id = action.getId();
        if (actions.containsKey(id)) {
            CsDebuggerMod.LOGGER.warn("Debug action {} is already registered, overwriting", id);
        }
        actions.put(id, action);
        CsDebuggerMod.LOGGER.info("Registered debug action: {}", id);
    }
